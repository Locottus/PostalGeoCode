...
const CoordenadasAGeopost = (request, response) => {
   const latitud = request.query.Latitude;
   const longitud = request.query.Longitude;
   var NS;
   var EO;
   var geocoordenada = '';
   ...
   response.status(200).json(JSON.parse('{"Geocode" : "' + geocoordenada + '"}'));
}


const GeopostACoordenadas = (request, response) => {
    
    const geocodigo = request.query.Geocode.toString().toUpperCase();
  ...
  //TERCER CALCULO DETERMINAR LOS DIGITOS DE LA MANTISA DEPNEDIENDO DE LOS VALORES DEL ARREGLO POSICIONES 1    
    ...
    response.status(200).json(JSON.parse('{"Latitude" : "' + latitud + '", "Longitude" : "'+longitud+'"}'));
 }
